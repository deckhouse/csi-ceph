...
# This webhook ensures the migration from CephClusterAuthentication to CephClusterConnection:
# - Copy userID and userKey from CephClusterAuthentication to CephClusterConnection
# - Remove field clusterAuthenticationName from CephStorageClass
# - Remove secret "csi-ceph-secret-for-<CephClusterAuthentication.Name>"
# - Recreate all PVs with provisioners rbd.csi.ceph.com and cephfs.csi.ceph.com: rename secretRef from csi-ceph-secret-for-<CephClusterAuthentication.Name> to csi-ceph-secret-for-<CephClusterConnection.Name>
# - Remove CephClusterAuthentication CRs
# - Mark CephClusterAuthentication CRD as migrated

# The expecting result of the hook is:
# - 
# - CephClusterAuthentication CRD has the condition 'MigrationCompleted' with status 'True'
# - CephClusterConnection CRs have the same userID and userKey as CephClusterAuthentication CRs
# - CephStorageClass CRs do not have the field clusterAuthenticationName
# - Secrets csi-ceph-secret-for-<CephClusterAuthentication.Name> do not exist
# - PVs with provisioners rbd.csi.ceph.com and cephfs.csi.ceph.com have secretRef csi-ceph-secret-for-<CephClusterConnection.Name>
# - CephClusterAuthentication CRs do not exist
