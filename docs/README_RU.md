...
{{< alert level="info" >}}
Снимки томов доступны только при подключенном модуле, поставляющем CRD группы `snapshot.storage.k8s.io`, — например [`snapshot-controller`](/modules/snapshot-controller/). Без него остальная функциональность модуля работает штатно, а VolumeSnapshotClass не создается.
{{< /alert >}}
