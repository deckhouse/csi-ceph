...
const (
	allowedUserName = "system:serviceaccount:d8-csi-ceph:controller"
)

var (
	managedStorageClassProvisioners = []string{
		"cephfs.csi.ceph.com",
		"rbd.csi.ceph.com",
	}
)
...
func SCValidate(_ context.Context, arReview *model.AdmissionReview, obj metav1.Object) (*kwhvalidating.ValidatorResult, error) {
	...
	if contains(managedStorageClassProvisioners, sc.Provisioner) {
		klog.Infof("Validating storage class with provisioner %s for user %s because it is in the list of managed provisioners: %v", sc.Provisioner, arReview.UserInfo.Username, managedStorageClassProvisioners)
		if arReview.UserInfo.Username == allowedUserName {
			klog.Infof("User %s is allowed to manage storage classes with provisioner %s", arReview.UserInfo.Username, sc.Provisioner)
		...
		if arReview.Operation == model.OperationUpdate {
			...
			if !changed {
				klog.Infof("User %s is allowed to change annotations for storage classes with provisioner %s", arReview.UserInfo.Username, sc.Provisioner)
				return &kwhvalidating.ValidatorResult{Valid: true},
					nil
			}
		}
